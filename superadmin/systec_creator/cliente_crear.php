...
// fallback si config.php no trae helper
...
if (!defined('SYSTEC_SEED_SUPERADMIN_NAME'))  define('SYSTEC_SEED_SUPERADMIN_NAME', 'Mikel SuperAdmin');
if (!defined('SYSTEC_SEED_SUPERADMIN_USER'))  define('SYSTEC_SEED_SUPERADMIN_USER', 'superadmin');
if (!defined('SYSTEC_SEED_SUPERADMIN_EMAIL')) define('SYSTEC_SEED_SUPERADMIN_EMAIL', 'mikeldng@c2k.cl');
if (!defined('SYSTEC_SEED_SUPERADMIN_PASS'))  define('SYSTEC_SEED_SUPERADMIN_PASS', '112233Kdoki.');

// Usuario del cliente (siempre)
if (!defined('SYSTEC_SEED_CLIENTADMIN_NAME_PREFIX')) define('SYSTEC_SEED_CLIENTADMIN_NAME_PREFIX', 'Admin ');
if (!defined('SYSTEC_SEED_CLIENTADMIN_USER'))        define('SYSTEC_SEED_CLIENTADMIN_USER', 'admin');
if (!defined('SYSTEC_SEED_CLIENTADMIN_PASS'))        define('SYSTEC_SEED_CLIENTADMIN_PASS', '112233');

if (!function_exists('sa_valid_slug')) {
    function sa_valid_slug(string $slug): bool {
        return (bool)preg_match('/^[a-z0-9_-]+$/', $slug);
    }
}
if (!function_exists('sa_valid_dbname')) {
    function sa_valid_dbname(string $db): bool {
        return (bool)preg_match('/^[a-zA-Z0-9_]+$/', $db);
    }
}
if (!function_exists('sa_sql_ident')) {
    function sa_sql_ident(string $ident): string {
        $ident = str_replace('`', '', $ident);
        return '`' . $ident . '`';
    }
}
if (!function_exists('sa_sql_quote')) {
...
if (!function_exists('sa_write_file')) {
...
if (!function_exists('sa_mkdir')) {
    function sa_mkdir(string $path): bool {
        if (is_dir($path)) return true;
        return mkdir($path, 0755, true);
    }
}
if (!function_exists('sa_client_pdo')) {
...
/* =========================
   ...
   ========================= */
if (!function_exists('sa_install_log_append')) {
...
if (!function_exists('sa_column_exists')) {
...
if (!function_exists('sa_index_exists')) {
...
/**
 * Tipo de objeto en DB: BASE TABLE / VIEW / null
 */
if (!function_exists('sa_table_type')) {
    function sa_table_type(PDO $pdoClient, string $name): ?string {
...
if (!function_exists('sa_ensure_usuarios_table')) {
...
if (!function_exists('sa_seed_usuario')) {
...
if (!function_exists('sa_starts_with')) {
 ...
 */
if (!function_exists('sa_sql_split_statements')) {
...
if (!function_exists('sa_schema_extract_version')) {
...
if (!function_exists('sa_ensure_schema_migrations')) {
...
if (!function_exists('sa_schema_record')) {
    function sa_schema_record(PDO $pdoClient, string $version, string $hash, string $by, string $notes): void {
        ...
        $st = $pdoClient->prepare("
            INSERT IGNORE INTO schema_migrations (schema_version, schema_hash, applied_by, notes)
            VALUES (:v, :h, :b, :n)
        ");
 ...
 */
if (!function_exists('sa_install_schema_from_file')) {
...
/**
 * Parches mínimos obligatorios (anti “Unknown column” + compat dashboard).
 */
if (!function_exists('sa_schema_patch_required')) {
    function sa_schema_patch_required(PDO $pdoClient, ?string $logPath = null): array {
        ...
        // Compat dashboard: VIEW ordenes_evidencias
        if (sa_table_exists($pdoClient, 'evidencias')) {
            $tt = sa_table_type($pdoClient, 'ordenes_evidencias');
            if ($tt === null || strtoupper($tt) === 'VIEW') {
                try {
                    $pdoClient->exec("
                        ...
                        FROM `evidencias`
                    ");
                    $notes[] = "VIEW ordenes_evidencias OK";
                    sa_install_log_append($logPath, "PATCH: create/replace VIEW ordenes_evidencias");
                } catch (Exception $e) {
                    sa_install_log_append($logPath, "WARN: could not create VIEW ordenes_evidencias: " . $e->getMessage());
                    $notes[] = "WARN view ordenes_evidencias";
                }
            } else {
                sa_install_log_append($logPath, "INFO: ordenes_evidencias exists as BASE TABLE (skip view).");
            }
        } else {
            sa_install_log_append($logPath, "INFO: evidencias table not found (skip ordenes_evidencias view).");
        }
 ...
 * - Si existe CORE v1.x/_db/schema.sql => lo aplica
 * - Si no existe => crea mínimo + parches
 */
if (!function_exists('sa_install_schema_auto')) {
    function sa_install_schema_auto(PDO $pdoClient, string $corePath, ?string $logPath = null): array {
        ...
        return [
            'ok' => true,
            'msg' => 'Schema fallback aplicado (mínimo) + ' . $patch['msg'],
            'version'=>'fallback-min',
            'hash'=>hash('sha256','fallback-min')
        ];
    }
}

/**
 * SQL manual (para phpMyAdmin) — incluye schema completo v1.2
 */
if (!function_exists('sa_build_install_sql')) {
    function sa_build_install_sql(string $slug, string $db_name, string $admin_email, string $admin_nombre): string {
        ...
        $sql[] = "  `schema_version` VARCHAR(60) NOT NULL,";
        $sql[] = "  `schema_hash` CHAR(64) NOT NULL,";
        $sql[] = "  `applied_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,";
        $sql[] = "  `applied_by` VARCHAR(60) NULL DEFAULT NULL,";
        $sql[] = "  `notes` VARCHAR(255) NULL DEFAULT NULL,";
        $sql[] = "  PRIMARY KEY (`id`),";
        $sql[] = "  UNIQUE KEY `uq_schema_version_hash` (`schema_version`, `schema_hash`),";
        $sql[] = "  KEY `idx_schema_version` (`schema_version`),";
        $sql[] = "  KEY `idx_schema_hash` (`schema_hash`),";
        $sql[] = "  KEY `idx_schema_applied_at` (`applied_at`)";
        ...
        $sql[] = "  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,";
        $sql[] = "  `nombre` VARCHAR(180) NOT NULL,";
        $sql[] = "  `rut` VARCHAR(20) NULL DEFAULT NULL,";
        $sql[] = "  `telefono` VARCHAR(40) NULL DEFAULT NULL,";
        $sql[] = "  `email` VARCHAR(180) NULL DEFAULT NULL,";
        $sql[] = "  `direccion` VARCHAR(255) NULL DEFAULT NULL,";
        $sql[] = "  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,";
        $sql[] = "  `updated_at` DATETIME NULL DEFAULT NULL,";
        $sql[] = "  PRIMARY KEY (`id`),";
        $sql[] = "  UNIQUE KEY `uq_clientes_rut` (`rut`)";
        $sql[] = ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;";
        $sql[] = "";
        ...
        $sql[] = "  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,";
        $sql[] = "  `cliente_id` INT UNSIGNED NULL DEFAULT NULL,";
        $sql[] = "  `tipo` VARCHAR(60) NULL DEFAULT NULL,";
        $sql[] = "  `marca` VARCHAR(60) NULL DEFAULT NULL,";
        $sql[] = "  `modelo` VARCHAR(80) NULL DEFAULT NULL,";
        $sql[] = "  `serie` VARCHAR(80) NULL DEFAULT NULL,";
        $sql[] = "  `imei` VARCHAR(30) NULL DEFAULT NULL,";
        $sql[] = "  `color` VARCHAR(40) NULL DEFAULT NULL,";
        $sql[] = "  `notas` TEXT NULL,";
        $sql[] = "  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,";
        $sql[] = "  `updated_at` DATETIME NULL DEFAULT NULL,";
        $sql[] = "  PRIMARY KEY (`id`),";
        $sql[] = "  KEY `idx_equipos_cliente` (`cliente_id`),";
        $sql[] = "  CONSTRAINT `fk_equipos_cliente`";
        $sql[] = "    FOREIGN KEY (`cliente_id`) REFERENCES `clientes`(`id`)";
        $sql[] = "    ON UPDATE CASCADE ON DELETE SET NULL";
        $sql[] = ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;";
        $sql[] = "";

        // ordenes (completa v1.2)
        $sql[] = "CREATE TABLE IF NOT EXISTS `ordenes` (";
        $sql[] = "  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,";
        $sql[] = "  `cliente_id` INT UNSIGNED NULL DEFAULT NULL,";
        $sql[] = "  `equipo_id` INT UNSIGNED NULL DEFAULT NULL,";
        $sql[] = "  `estado_actual` VARCHAR(30) NOT NULL DEFAULT 'INGRESADA',";
        $sql[] = "  `prioridad` VARCHAR(15) NOT NULL DEFAULT 'NORMAL',";
        $sql[] = "  `fecha_ingreso` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,";
        $sql[] = "  `fecha_entrega_estimada` DATETIME NULL DEFAULT NULL,";
        $sql[] = "  `fecha_entrega_real` DATETIME NULL DEFAULT NULL,";
        $sql[] = "  `detalle_falla` TEXT NULL,";
        $sql[] = "  `diagnostico` TEXT NULL,";
        $sql[] = "  `solucion` TEXT NULL,";
        $sql[] = "  `observaciones` TEXT NULL,";
        $sql[] = "  `total` DECIMAL(12,2) NOT NULL DEFAULT 0.00,";
        $sql[] = "  `abono` DECIMAL(12,2) NOT NULL DEFAULT 0.00,";
        $sql[] = "  `saldo` DECIMAL(12,2) NOT NULL DEFAULT 0.00,";
        $sql[] = "  `created_by` INT UNSIGNED NULL DEFAULT NULL,";
        $sql[] = "  `updated_by` INT UNSIGNED NULL DEFAULT NULL,";
        $sql[] = "  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,";
        $sql[] = "  `updated_at` DATETIME NULL DEFAULT NULL,";
        $sql[] = "  PRIMARY KEY (`id`)";
        $sql[] = ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;";
        ...
        $sql[] = "  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,";
        $sql[] = "  `orden_id` INT UNSIGNED NOT NULL,";
        $sql[] = "  `tipo` VARCHAR(30) NOT NULL DEFAULT 'IMG',";
        $sql[] = "  `ruta` VARCHAR(255) NOT NULL,";
        $sql[] = "  `descripcion` VARCHAR(255) NULL DEFAULT NULL,";
        $sql[] = "  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,";
        $sql[] = "  PRIMARY KEY (`id`)";
        $sql[] = ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;";
        $sql[] = "";
        ...
        // compat view
        $sql[] = "CREATE OR REPLACE VIEW `ordenes_evidencias` AS";
        $sql[] = "SELECT `id`,`orden_id`,`tipo`,`ruta`,`descripcion`,`created_at` FROM `evidencias`;";
        $sql[] = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $core_version_in  = sa_post('core_version', 'v1.2');
    ...
    if ($mode === 'create') {
        ...
        if (!defined('SYSTEC_ROOT') || !SYSTEC_ROOT) {
            $errors[] = 'SYSTEC_ROOT no está definido en _config/config.php';
        }
        if (!defined('SYSTEC_CLIENTS_ROOT') || !SYSTEC_CLIENTS_ROOT) {
            $errors[] = 'SYSTEC_CLIENTS_ROOT no está definido en _config/config.php';
        }

        // Validar core_version exista físicamente
        $corePath = (defined('SYSTEC_ROOT') && SYSTEC_ROOT) ? (SYSTEC_ROOT . '/_cores/systec/' . $core_version) : '';
        ...
        if (empty($errors)) {
            ...
            $fsClientRoot = (defined('SYSTEC_CLIENTS_ROOT') && SYSTEC_CLIENTS_ROOT) ? (SYSTEC_CLIENTS_ROOT . '/' . $slug) : '';
...
PHP;
                ...
                if (empty($errors)) {
                    try {
                        ...
                        // 3) Parches obligatorios (anti “Unknown column” + VIEW compat)
                        ...
                        sa_flash_set('clientes', 'Cliente creado: ' . $slug . $note, 'success');
      ...
      <div class="card-body">
        ...
        <form method="post" autocomplete="off">
          ...
          <div class="form-row">
            ...
            <div class="form-group col-md-3">
              <label>DB User *</label>
              <input type="text" name="db_user" class="form-control" value="<?php echo htmlspecialchars($db_user, ENT_QUOTES, 'UTF-8'); ?>" required autocomplete="new-password">
            </div>
            <div class="form-group col-md-3">
              <label>DB Pass *</label>
          ...
          <div class="text-muted small mb-2">
            Se crea automáticamente el usuario <code>admin</code> con contraseña provisoria <code>112233</code>.
          </div>
