...
if ($id <= 0) exit('ID de orden no válido');
...
// Base URL REAL (APP_URL puede venir como "/SysTec..." o vacío)
$scheme  = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https://' : 'http://';
$appPath = rtrim((string)APP_URL, '/');
...
/* -----------------------------
   Evidencias visibles para cliente (base64)
----------------------------- */
$stmtEv = $pdo->prepare("
...
/* -----------------------------
   Reglas de paginado evidencias
   - Página 1: máximo 2 evidencias
   - Páginas siguientes: resto (en galería)
----------------------------- */
$eviFirstPageMax = 2;
...
function renderGaleriaEvidencias($items) {
    ...
    foreach ($items as $ev) {
        ...
        if ($coment !== '') $html .= '  <div class="evi-com muted">' . $coment . '</div>';
        if ($fecha !== '')  $html .= '  <div class="evi-fecha muted">' . $fecha . '</div>';
        $html .= '    </div>';
        $html .= '  </div>';
        $html .= '</td>';
    ...
    // Si quedó fila incompleta, cerramos con celda vacía
    if ($col === 1) {
...
<head>
    ...
    <style>
        * { box-sizing: border-box; font-family: DejaVu Sans, Arial, Helvetica, sans-serif; font-size: 10px; }
        ...
        /* Evidencias: tabla 2 columnas (estable en Dompdf) */
        ...
        /* salto limpio a página solo evidencias */
